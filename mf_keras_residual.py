...
from tensorflow.keras.layers import Input, Embedding, Dense, Concatenate, Dot, Add, Flatten, Activation
import warnings
...
class MatrixFactorization:
    ...
    def fit(self, X_train, X_test, epochs=10, lr=0.01):
        ...
        mf = Dot(axes=2)([P, Q])
        mf = Add()([mf, user_bias, movie_bias])
        mf = Flatten()(mf)
        
        ann = Concatenate()([P, Q])
        ann = Flatten()(ann)
        ann = Dense(4)(ann)
        ann = Activation('relu')(ann)
        ann = Dense(1)(ann)
    ...
    def predict(self, X):
        inputs = [X['userId'], X['movieId']]
        ratings = self.model.predict(inputs)
    ...
    def evaluate(self, X):
        ...
        print_debug(pred.shape)
        print_debug(X['rating'].shape)
        e = X['rating'] - pred
        return np.mean(e ** 2)
...
train, test = train_test_split(R, test_size=0.01)
